...
function renderPlaces(places, preserveSource = false) {
  ...
  places.forEach(place => {
    ...
    btn.classList.toggle('border-emerald-300', selected);
    btn.innerHTML = `<div class="flex flex-col">` +
      `<span class="title">${name}</span>` +
      `<span class="meta">${option.dataset.address || 'Адрес не указан'}</span>` +
      `</div><div class="text-[11px] text-slate-500 flex items-center gap-1">${selected ? '<span class="pill green text-xs">выбрано</span>' : ''}<span>${placeId}</span></div>`;
    btn.addEventListener('click', (evt) => { evt.preventDefault(); togglePlaceSelection(placeId); });
...
function renderMenuTable(rows) {
  ...
  placeGroups.forEach((itemsForPlace, placeName) => {
    ...
    placeHeader.innerHTML = `<th colspan="8" class="px-3 py-2 text-left w-full">${placeCollapsed ? '➕' : '➖'} ${placeName}</th>`;
    ...
    grouped.forEach((items, category) => {
      ...
      header.innerHTML = `<th colspan="8" class="px-3 py-2 font-semibold text-slate-800 w-full">${catCollapsed ? '➕' : '➖'}
        <span class="pill blue ml-2">${category || 'Без категории'}</span>
...
function renderStopListPanel() {
  ...
  const grouped = new Map();
  stops.forEach(item => {
    const place = item.place || 'Без точки';
  ...
  const fragments = [];
  grouped.forEach((catMap, place) => {
    fragments.push(`<tr class="group-city-row"><th colspan="5" class="text-left px-3 py-2">${place}</th></tr>`);
    catMap.forEach((items, cat) => {
      fragments.push(`<tr class="group-category-row"><th colspan="5" class="text-left px-3 py-2">${cat}</th></tr>`);
      items.forEach(r => {
        ...
        fragments.push(`<tr class="border-b border-slate-100" data-search="${[place, cat, r.name, r.id].join(' ').toLowerCase()}">
          ...
          <td class="px-2 py-1 text-center">${formatPrice(r.price)}</td>
          <td class="px-2 py-1 text-center">${r.stopDate || ''}</td>
        </tr>`);
      });
    });
  });
  stopTableBody.innerHTML = fragments.join('') || `<tr><td colspan="5" class="px-2 py-2 text-center text-slate-500">Стоп-лист пуст.</td></tr>`;
...
function filterStopList(term) {
  ...
  Array.from(stopTableBody.querySelectorAll('tr')).forEach(row => {
    ...
    row.classList.toggle('hidden', !!value && !searchable.includes(value));
  });
}
...
function showDishOverlay(row, fullImg) {
  ...
  (row.modifierGroups || []).forEach(g => {
    const block = document.createElement('div');
    block.className = 'border border-slate-200 rounded-md p-3 bg-slate-50 text-center';
    ...
    block.innerHTML = `<div class="font-semibold text-indigo-800">${g.name || 'Группа модификаторов'}</div>
      <div class="text-[11px] text-slate-500 mb-2">мин ${g.min ?? 0} / макс ${g.max ?? 0}${g.required ? ' (обязательно)' : ''}</div>
      ${modsHtml}`;
