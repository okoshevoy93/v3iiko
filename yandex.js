...
function renderPlaces(places, preserveSource = false) {
  ...
  places.forEach(place => {
    ...
    const active = selectedPlaceIds.has(placeId);
    btn.type = 'button';
    ...
    btn.innerHTML = `<div class="flex flex-col">
      ...
      <div class="text-[11px] text-slate-500 flex items-center gap-1">${placeId}</div>`;
    btn.addEventListener('click', (ev) => { ev.stopPropagation(); togglePlaceSelection(placeId); });
...
function buildAvailabilityMap(data) {
  ...
  const items = Array.isArray(data)
    ...
    items.forEach(entry => {
      ...
      if (!id) return;
      let available = entry.available ?? entry.is_available ?? entry.in_stock;
      if (entry.in_stop_list === true || entry.stop_list === true || entry.is_blocked === true) {
        available = false;
      }
      const quantity = entry.balance ?? entry.quantity ?? entry.count ?? entry.stock ?? null;
      const stopDate = entry.stopDate || entry.stop_date || entry.stopped_at || entry.updated_at || entry.updatedAt || entry.timestamp || entry.date || entry.lastChange || entry.changed_at || '';
      map.set(id, { available, quantity, stopDate });
    });
  return map;
}
...
function normalizeMenuItems(menuName, menuData, availabilityMap, meta = {}) {
  ...
  const mapped = (items || []).map(item => {
    ...
    return {
      ...
      stopDate: availability.stopDate || item.stopDate || item.lastChange || item.changedAt || item.changed_at || '',
      modifiers,
      modifierGroups,
      modifierCount,
...
function updateSummaryFromRows(currentRows = null) {
  ...
  source.forEach(row => {
    ...
    const groupMods = (row.modifierGroups || []).reduce((acc, g) => acc + (g.modifiers?.length || 0), 0);
    modifiers += groupMods + (row.modifiers?.length || 0);
...
function renderMenuTable(rows) {
  ...
  const colCount = 8;
  const sortedPlaces = Array.from(placeGroups.keys());
  sortedPlaces.forEach(placeName => {
    const itemsForPlace = placeGroups.get(placeName) || [];
    ...
    placeHeader.innerHTML = `<th colspan="${colCount}" class="group-cell">` +
      `<div class="flex items-center gap-2"><span class="pill-icon">${placeCollapsed ? '➕' : '➖'}</span><span>Город: ${placeName}</span><span class="text-slate-200">(${itemsForPlace.length})</span></div>` +
      `</th>`;
    ...
    const sortedCats = Array.from(grouped.keys());
    sortedCats.sort((a, b) => {
      if (sortState.field === 'category') {
        return sortState.dir === 'asc' ? a.localeCompare(b) : b.localeCompare(a);
      }
      return 0;
    });
    ...
    sortedCats.forEach(category => {
      ...
      header.innerHTML = `<th colspan="${colCount}" class="group-cell pl-8">` +
        `<div class="flex items-center gap-2"><span class="pill-icon">${catCollapsed ? '➕' : '➖'}</span><span>${category || 'Без категории'}</span><span class="text-slate-500">(${items.length})</span></div>` +
        `</th>`;
      ...
      if (sortState.field === 'name') {
        sortedItems.sort((a, b) => sortState.dir === 'asc'
      ...
      sortedItems.forEach(row => {
        ...
        tr.className = `menu-data-row ${row.stopList ? 'bg-rose-50/60' : ''} ${catId} ${placeClass}`;
        ...
        tr.innerHTML = `
            ...
            <td class="px-3 py-2 flex items-center gap-2 justify-center align-middle text-center">${row.name} ${stopBadge}</td>
...
function applyAvailabilityToRows(placeId, availabilityMap) {
  if (!availabilityMap || !(availabilityMap instanceof Map)) return;
  normalizedMenuRows = normalizedMenuRows.map(row => {
    const matchPlace = placeId ? row.placeId === placeId : true;
    if (!matchPlace) return row;
    const entry = availabilityMap.get(row.id);
    if (!entry) return row;
    const available = entry.available ?? row.available;
    return {
      ...row,
      available,
      stopList: available === false,
      stopDate: entry.stopDate || row.stopDate,
...
function renderStopListPanel() {
  ...
  stops.forEach(row => {
    ...
    if (!placeGroups.has(place)) placeGroups.set(place, new Map());
    const catMap = placeGroups.get(place);
    if (!catMap.has(cat)) catMap.set(cat, []);
    catMap.get(cat).push(row);
  });

  const fragments = [];
  placeGroups.forEach((catMap, placeName) => {
    fragments.push(`<tr class="group-city-row"><th colspan="5" class="px-3 py-2">${placeName}</th></tr>`);
    catMap.forEach((rows, catName) => {
      fragments.push(`<tr class="group-category-row"><th colspan="5" class="px-3 py-2">${catName}</th></tr>`);
      rows.forEach(r => {
        ...
        const date = r.stopDate || '—';
        fragments.push(`<tr class="border-b border-slate-100"><td class="px-2 py-1">${img}</td><td class="px-2 py-1">${r.name}</td><td class="px-2 py-1 text-xs text-slate-500">${r.id || ''}</td><td class="px-2 py-1">${formatPrice(r.price)}</td><td class="px-2 py-1 text-xs text-slate-500">${date}</td></tr>`);
      });
    });
  });

  stopTableBody.innerHTML = fragments.join('');
  stopOverlay.classList.add('active');
}

function filterStopList(term) {
  if (!stopTableBody) return;
  const value = (term || '').toLowerCase();
  Array.from(stopTableBody.querySelectorAll('tr')).forEach(row => {
    if (!value) { row.classList.remove('hidden'); return; }
...
function restoreSession() {
  ...
  try {
    ...
    if (state.sortState) sortState = state.sortState;
...
function showDishOverlay(row, fullImg) {
  ...
  groups.forEach(g => {
    ...
    block.innerHTML = `<div class="font-semibold text-slate-800 text-center">${g.name || 'Группа модификаторов'}</div>
  ...
  if (!groups.length && !(row.modifiers || []).length) {
...
async function runAvailabilityFor(placeId, renderList = false) {
  ...
  try {
    ...
    const map = buildAvailabilityMap(data);
    applyAvailabilityToRows(restaurantId, map);
    renderMenuTable(renderedMenuRows.length ? renderedMenuRows : normalizedMenuRows);
    updateSummaryFromRows();
    if (renderList) {
      renderStopListPanel();
    } else {
      const items = normalizedMenuRows.filter(row => map.has(row.id) && map.get(row.id)?.available === false);
      const list = items.map(item => `- ${item.category} — ${item.name} (${item.id}) ${item.price || ''}`).join('\n');
      renderExtraResult('Недоступные позиции', list || data);
    }
...
stopListBtn?.addEventListener('click', async () => {
  ...
  for (const id of ids) {
    await runAvailabilityFor(id, true);
  }
  renderStopListPanel();
});
...
function updateSortLabels() {
  if (thCategory) thCategory.textContent = `Категория ${sortState.field === 'category' ? (sortState.dir === 'asc' ? '▲' : '▼') : '▲▼'}`;
  if (thName) thName.textContent = `Блюдо / товар ${sortState.field === 'name' ? (sortState.dir === 'asc' ? '▲' : '▼') : '▲▼'}`;
}
